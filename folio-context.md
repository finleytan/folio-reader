...
Single-file HTML PWA (~3,872 lines). Audiobook/ebook reader with synced word-level highlighting.
...
**Approximate line ranges (index.html):** CSS `16–511` · HTML `513–914` · JS `915–3870`
...
| `#modal` | Add Book modal (dropzone, file pills: Audio, Ebook, Transcript, Cover; folder assign with tiered JSON auto-detection) |
...
| **DISPLAY PREFERENCES** | `saveDisplayPrefs()` (reads `_fontBody/_fontSize/_lineHeight/_maxWidth`; saves `sentHl`/`wordHl`/`hlMode`; no `getComputedStyle`), `loadDisplayPrefs()` (restores `sentHlOn`/`wordHlOn` from booleans for backward compat, then overrides with `hlMode` if present; calls `_syncHlPills()`; migrates old `'Open Dyslexic'` → `'OpenDyslexic'` in saved fontBody; if saved `fontKey==='dyslexic'`, injects CDN `<link>` with `onload` repaint — same guard as `setFont()`) |
...
| **OPTIONS PANEL** | `toggleOpts()`, `switchOptTab()`, `setTheme()`, `updateThemeColor()`, `setFont()` (on-demand CDN injection for OpenDyslexic when `key==='dyslexic'` via jsdelivr @fontsource/opendyslexic; loads both 400 and 700 weights; guarded by `getElementById` to prevent duplicate `<link>`; `link.onload` re-sets `--font-body` to force repaint after stylesheet arrives), `setFS/LH/MW()`, `setAlign()`, `setDefaultWpmFromSlider()`, `setDefaultWpmFromInput()`, `updateWpmLabel()`, `setSentPause()`, `toggleOpInfo(el)` (toggles `.op-desc` sibling of nearest `.op-ttl`/`.op-row` ancestor), scroll-pause IIFE (rAF-throttled), click-outside handler. WPM slider/input/label HTML removed from Playback tab (JS functions kept for `loadDisplayPrefs`/`saveDisplayPrefs` compat) |
| **TRANSCRIPT** | `loadTranscriptData()` (handles segment-level Whisper JSON without word_timestamps; validates JSON structure — rejects files without `.segments` or `.words`), `setBannerState()` (auto-hides 'ready' state after 12s with fade via `_bannerHideTimer`), `_timingWorkerFn()` (serialised into Blob Worker; handles `'ping'`, `'buildSentenceTimings'`, `'buildTimingsFromPlainText'` message types), `getTimingWorker()` (try/catch around Worker creation — returns `null` on failure instead of throwing; `onerror` clears `_timingWorkerTimeout` and nulls `_timingWorker` before sync fallback), `buildSentenceTimings()` (worker dispatch; sets `_pendingTimingBookIdx` before banner/yield; 20-second watchdog; graceful fallback if `getTimingWorker()` returns null), `buildTimingsFromPlainText()` (worker dispatch; sets `_pendingTimingBookIdx` before banner/yield), `_buildSentenceTimingsSync()` (fallback), `_buildTimingsFromPlainTextSync()` (fallback), `similarity()`, `updateTranscriptUI()`; after timing is built, `transcriptWords` and `transcriptText` are nulled (both worker onmessage path and sync fallback); worker onmessage calls `showSyncHintOnce()` only on `type==='buildSentenceTimings'` (Whisper word-level), not plain-text path. Matching tuning: search window `sWords.length*30+200` (both passes), `maxRunLen` capped at `sWords.length+15` for ≤6-word sentences, pass 1 threshold tiered (`≤4` words→0.45, `≤7`→0.52, else 0.6) |
...
| **ADD BOOK MODAL** | `openModal()`, `closeModal()`, `resetModal()`, file/folder handlers, `folderAssign()`, `folderChosen()` (tiered JSON transcript auto-assignment matching PWA logic: Tier 1 'transcript'/'whisper' in filename, Tier 2 folder-name word match 3+ chars, Tier 3 lone JSON fallback), `addBook()`. Modal has 4 file pills: Audio, Ebook, Transcript (optional), Cover (optional) |
| **TRANSCRIPT MODAL** | `openTranscriptModal()`, `saveTranscript()`, `removeTranscript()` |
| **LINK AUDIO MODAL** | `openLinkAudioModal()`, `saveLinkAudio()` (after linking audio: shows speed strip, removes `.scrubbing` class from progress bar — mirrors `configurePlayerForMode` audio-mode setup) |
...
| `_timingWorkerTimeout` | setTimeout ID for 20-second worker watchdog in `buildSentenceTimings`; cleared on worker response, worker error, or `goLib()` |
...
buildSentenceTimings()
  └── sets _pendingTimingBookIdx=curBookIdx → setBannerState('syncing') → posts {sentTexts, sentWordCounts, transcriptWords, bookIdx}
      to inline Blob Worker via getTimingWorker() (if null, falls back to sync immediately).
      After postMessage, starts a 20-second watchdog (_timingWorkerTimeout): if the worker
      doesn't respond in time, terminates it, updates banner, falls back to _buildSentenceTimingsSync().
...
28. **`resetModal()` must match Add Book modal HTML pill IDs**: `resetModal()` iterates `['audioName','ebookName','transcriptName','coverName']` and sets `.textContent` on each. If a pill is removed from or added to the modal HTML, this array must be updated in sync — a missing element causes `null.textContent` TypeError that kills `openModal()`.
29. **Transcript JSON auto-assignment is duplicated**: Tiered matching logic (Tier 1 'transcript'/'whisper', Tier 2 folder-name word 3+ chars, Tier 3 lone JSON) exists in both `folderChosen()` (browser mode) and `pwaScanBookFolder()` (PWA mode). Changes to the tiering heuristics must be made in both places. The browser version uses `File` objects with `.name`; the PWA version uses `{name, handle, ext}` objects — the `.name` property is compatible but the structures differ.
30. **`getTimingWorker()` can return `null`**: Worker creation is wrapped in try/catch — `file://` origins, CSP restrictions, or browser bugs can prevent Blob Worker instantiation. All callers (`buildSentenceTimings`, `buildTimingsFromPlainText`) must null-check the return value and fall back to `_buildSentenceTimingsSync()` / `_buildTimingsFromPlainTextSync()`. The `onerror` handler also nulls `_timingWorker` and clears `_timingWorkerTimeout` before falling back.
31. **`saveLinkAudio()` must mirror `configurePlayerForMode` audio-mode setup**: When linking audio to a TTS-only book, `saveLinkAudio()` shows the seek strip and hides the TTS bar, but must also show the speed strip (`.speed-strip`) and remove the `.scrubbing` class from `.read-progress-wrap`. Missing either leaves TTS-mode UI artifacts visible in audio mode.
32. **`_pendingTimingBookIdx` must be set before `yieldToMain()`**: Both `buildSentenceTimings()` and `buildTimingsFromPlainText()` set `_pendingTimingBookIdx=curBookIdx` before the first `await`. If set after `yieldToMain()`, a rapid book switch during the yield can cause the stale-result guard to accept results for the wrong book.
